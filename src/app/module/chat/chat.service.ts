...
const getChatMessages = async (
  ...
  // A chat is addressed by id alone, so without this any authenticated caller
  // could read a conversation they are not part of.
  const isParticipant = chat.participants.some(
    (p: any) => String(p?._id ?? p) === userData.userId,
  );
  if (!isParticipant)
    throw new ApiError(status.FORBIDDEN, "This is not your conversation");
...
const updateMessageAsSeen = async (
  ...
  if (!chat.participants.some((p: any) => String(p) === userId))
    throw new ApiError(status.FORBIDDEN, "This is not your conversation");
