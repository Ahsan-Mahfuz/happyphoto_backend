...
const registrationAccount = async (payload: {
  ...
  // Self-service registration must never mint an admin: role comes straight from
  // the request body, so allowing ADMIN here hands anyone the admin dashboard.
  // Admins are provisioned by the seeder (see src/connection/seedAdmin.ts).
  if (role === EnumUserRole.ADMIN)
    throw new ApiError(status.FORBIDDEN, "Cannot register as admin");
